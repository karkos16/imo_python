...
def solve_steepest_edge(starting_routes, distances, _=None):
    """
    Steepest descent optimization: tries best edge flip or vertex exchange between two routes.
    """
            ...
            if use_edge_method:
                if best_in_route2:
                    if abs(best_i - best_j) == 1:
                        ...
                        route2[best_i:best_j] = route2[best_i:best_j][::-1]
                else:
                    if abs(best_i - best_j) == 1:
                        route1[best_i], route1[best_j] = route1[best_j], route1[best_i]
                    else:
                        route1[best_i:best_j] = route1[best_i:best_j][::-1]
            else:
