...
def solve_lns(distances):
    ...
    def destroy_solution(routes):
        ...
        if np.any(np.array(route1) == np.array(route2)):
            print("Error: routes have same cities -- before destroy")
            exit()

        if len(route1) != len(set(route1)):
            print(len(route1), len(set(route1)))
            print("Error: route1 has duplicates -- before destroy")
            print(f"Route 1: {route1}")
            unique, counts = np.unique(route1, return_counts=True)
            duplicates = unique[counts > 1]
            print(f"Duplicates: {duplicates}")
            exit()
        
        if len(route2) != len(set(route2)):
            print("Error: route2 has duplicates -- before destroy")
            print(f"Route 2: {route2}")
            unique, counts = np.unique(route2, return_counts=True)
            duplicates = unique[counts > 1]
            print(f"Duplicates: {duplicates}")
            exit()
    ...
    def repair_solution(routes, removed_cities):
        ...
        # Check if routes has same cities
        if np.any(np.array(route1) == np.array(route2)):
            print("Error: routes have same cities -- after repair")
            print(f"Route 1: {route1}")
            print(f"Route 2: {route2}")
            exit()

        if len(route1[:-1]) != len(set(route1[:-1])):
            print("Error: route1 has duplicates -- after repair")
            exit()
        
        if len(route2[:-1]) != len(set(route2[:-1])):
            print("Error: route2 has duplicates -- after repair")
            exit()
    ...
    print(f"Route 1 length: {len(best_solution[0])}, Route 2 length: {len(best_solution[1])}")
